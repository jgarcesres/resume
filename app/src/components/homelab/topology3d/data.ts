...
export const NODES: TopoNode[] = [
  // ── Florida ──
  { id: 'fl-pve1', label: 'fl-pve1', sub: 'Lenovo P520 · Proxmox VE 8', kind: 'hypervisor', site: 'fl', tier: 'phys',
  ...
  { id: 'truenas', label: 'truenas', sub: 'VMID 104 · on fl-pve1', kind: 'vm', site: 'fl', tier: 'vm',
    ...
    meta: { Resources: '48 GB RAM', HBA: 'SATA controller passthrough', NFS: 'movies · series · downloads', iSCSI: 'democratic-csi targets' } },
  { id: 'k3s-pve-vm', label: 'VM 107', sub: 'k3s-pve · on fl-pve1', kind: 'vm', site: 'fl', tier: 'vm',
    pos: [-14.2, -0.2], size: [1.5, 0.85, 1.7],
    meta: { RAM: '32 GB', GPU: 'RTX 4000 · passthrough', Storage: 'dedicated storage bridge · MTU 9000' } },
  { id: 'k3s-pve', label: 'k3s-pve', sub: 'worker · GPU · public ingress', kind: 'worker', site: 'fl', tier: 'k3s',
    pos: [-14.2, -0.2], r: 1.15, h: 0.8,
    ts: { host: 'k3s-pve', tags: 'tag:k8s' },
  ...
  { id: 'mde-pve1', label: 'mde-pve1', sub: 'ASRock Rack X570D4U-2L2T · Proxmox VE', kind: 'hypervisor', site: 'mde', tier: 'phys',
    pos: [4.2, -0.2], size: [4.2, 1.5, 2.8], bays: 2, uplink: [1.9, -1.2],
    ts: { host: 'mde-pve1', tags: 'tag:proxmox' },
    note: 'Board swapped to the X570D4U-2L2T on 2026-09-12 — first BMC in either rack.',
    meta: { CPU: 'Ryzen 9 5950X · 16C/32T', Storage: '1 TB 970 EVO Plus', NIC: '10GBase-T ×2', BMC: 'IPMI' } },
  ...
  { id: 'cp1-hw', label: 'NUC8i7', sub: 'k3s-cp1 host · fanless', kind: 'mini', site: 'mde', tier: 'phys',
    ...
    meta: { CPU: 'i7-8559U · 4C/8T', Disk: 'NVMe · LVM', Chassis: 'passive' } },
  ...
  { id: 'k3s-cp1', label: 'k3s-cp1', sub: 'control · etcd · cluster-init', kind: 'cp', site: 'mde', tier: 'k3s',
    ...
    ts: { host: 'k3s-cp1', tags: 'tag:k8s' },
    meta: { Role: 'control-plane + etcd', Join: 'cluster-init member', Taint: 'control-plane:NoSchedule' } },
  { id: 'k3s-cp2', label: 'k3s-cp2', sub: 'control · etcd', kind: 'cp', site: 'mde', tier: 'k3s',
    pos: [12.2, -1.2], r: 0.95, h: 0.7,
    ts: { host: 'k3s-cp2', tags: 'tag:k8s' },
    meta: { Role: 'control-plane + etcd', Hosts: 'monitoring · egress proxies (floating)', Taint: 'control-plane:NoSchedule' } },
  { id: 'k3s-cp3', label: 'k3s-cp3', sub: 'control · etcd · Zen 3', kind: 'cp', site: 'mde', tier: 'k3s',
    pos: [14.8, -1.2], r: 0.95, h: 0.7,
    ts: { host: 'k3s-cp3', tags: 'tag:k8s' },
    meta: { Role: 'control-plane + etcd', Bench: '~3.3× cp2 on sysbench', Taint: 'control-plane:NoSchedule' } },
  { id: 'mde-k3s-w2', label: 'mde-k3s-w2', sub: 'worker · arm64', kind: 'worker', site: 'mde', tier: 'k3s',
    pos: [17.4, -1.2], r: 0.8, h: 0.6,
    ts: { host: 'mde-k3s-w2', tags: 'tag:k8s' },
...
export const CHIPS: Chip[] = [
  ...
  { id: 'ingress-mde', label: 'ingress-mde', sub: 'Tailscale ingress · MDE control plane', kind: 'ingress', site: 'mde',
    pos: [12.2, -3.9], hosts: ['k3s-cp1', 'k3s-cp2', 'k3s-cp3'],
  ...
  { id: 's3-egress', label: 'mde-s3-egress', sub: 'Tailscale egress → Garage', kind: 'egress', site: 'mde',
    pos: [8.2, 1.6], hosts: ['k3s-cp2'], target: 's3', targetKind: 'backup',
    meta: { Placement: 'floats across MDE nodes', Consumers: 'Loki · Thanos · VolSync · CNPG · etcd' } },
  { id: 'ollama-egress', label: 'tinybeast-ollama', sub: 'Tailscale egress → Ollama', kind: 'egress', site: 'mde',
    pos: [19.2, 1.2], hosts: [], target: 'tinybeast', targetKind: 'tailnet',
    meta: { Path: 'LiteLLM → Ollama on tinybeast' } },
];
...
export const LINKS: LinkDef[] = [
  ...
  L('switch', 's3', 'lan', 'PoE'), L('switch', 'mde-kvm', 'lan', 'PoE'), L('switch', 'w2-hw', 'lan', 'PoE'),
